        ...
        addCommands(
                ...
                new ParallelCommandGroup(
                        new SequentialCommandGroup(
                                ...
                                new InstantCommand(() -> drivetrainSubsystem.setPose(
                                        PathPlannerPath.fromPathFile("homeTo31").getPreviewStartingHolonomicPose()))),
                        new SequentialCommandGroup(
                                new ParallelCommandGroup(
                                        new SetArmAngleCommand(mArm, ShootingParameters.BELOW_SPEAKER.angle_deg),
                                        new SetShooterTargetCommand(mShooter,
                                                ShootingParameters.BELOW_SPEAKER.speed_rps)),
                                new FeedCommand(mTransfer),
                                new InstantCommand(() -> mShooter.stop()))),
                // go 31 and back shoot
                buildPath("homeTo31")
                        .deadlineWith(new IntakeCommand(mIntake, mTransfer)),
                buildPath("31ToHome")
                        .alongWith(new SetShooterTargetCommand(mShooter, ShootingParameters.BELOW_SPEAKER.speed_rps))
