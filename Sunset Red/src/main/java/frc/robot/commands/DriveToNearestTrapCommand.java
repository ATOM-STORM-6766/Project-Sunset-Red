...
public class DriveToNearestTrapCommand extends Command {
    ...
    private Pose2d findNearestTrapPose(Pose2d currentPose) {
        ...
        for (int tagId : trapTagIds) {
            ApriltagCoprocessor.getInstance().aprilTagFieldLayout.getTagPose(tagId).ifPresent(
                tagPose -> trapPoses.add(tagPose.toPose2d())
            );
        }
        ...
        return nearestTrap.transformBy(new Transform2d(new Translation2d(DISTANCE_OFFSET, 0), new Rotation2d()));
    }
}