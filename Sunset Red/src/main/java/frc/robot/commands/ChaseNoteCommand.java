package frc.robot.commands;
 ...
 public class ChaseNoteCommand extends Command {
     ...
     private static final double INTAKE_STATE_TIMEOUT = 2.0; // INTAKE will be considered failed if it takes longer than
                                                             // this
     private static final double STATE_TIMEOUT = 5.0; // other states will be considered failed if they take longer than
                                                      // this
     ...
     private static final double CLOSE_PITCH_THRESHOLD = 3; // todo: measure this during field measurement
     ...
     public ChaseNoteCommand(DrivetrainSubsystem sDrivetrainSubsystem, Intake sIntake, Transfer sTransfer, Arm sArm) {
         this.sDrivetrainSubsystem = sDrivetrainSubsystem;
         this.sIntake = sIntake;
         this.sTransfer = sTransfer;
         this.sArm = sArm;

         xController = new PIDController(X_CONTROLLER_CONSTANTS[0], X_CONTROLLER_CONSTANTS[1],
                 X_CONTROLLER_CONSTANTS[2]);
         yController = new PIDController(Y_CONTROLLER_CONSTANTS[0], Y_CONTROLLER_CONSTANTS[1],
                 Y_CONTROLLER_CONSTANTS[2]);
         rotController = new PIDController(ROT_CONTROLLER_CONSTANTS[0], ROT_CONTROLLER_CONSTANTS[1],
                 ROT_CONTROLLER_CONSTANTS[2]);
     ...
     private void executeIntaking() {
         sIntake.setIntake();
         sTransfer.setVoltage(Transfer.INTAKE_VOLTS);
         sDrivetrainSubsystem.drive(new Translation2d(INTAKE_STATE_DRIVE_SPEED, 0), 0, false); // in intake state, we
                                                                                               // only drive forward,
                                                                                               // assume we are close
                                                                                               // enough to the note
     ...
     private ChassisSpeeds calculateChaseSpeed(double pitchMeasure, double yawMeasure) {
         ...
         var yOutput = yController.calculate(yawMeasure, 0); // camera centered: note is always in the middle of the
                                                             // camera view
     ...
     private void updateDashboard() {
         ...
         SmartDashboard.putBoolean("Target Present",
