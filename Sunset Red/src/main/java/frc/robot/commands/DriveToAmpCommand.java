...
public class DriveToAmpCommand extends Command {
    ...
    private Pose2d findAmpPose(Pose2d currentPose) {
        ...
        Optional<Pose3d> ampTagPoseOptional = ApriltagCoprocessor.getInstance().aprilTagFieldLayout.getTagPose(ampTagId);
        
        if (ampTagPoseOptional.isPresent()) {
            Pose2d ampPose = ampTagPoseOptional.get().toPose2d();
            return ampPose.transformBy(new Transform2d(new Translation2d(DISTANCE_OFFSET,0 ), Rotation2d.fromDegrees(180)));
        } else {
