...
import frc.robot.auto.modes.California.CaliforniaAutoScore515253;
...
public class RobotContainer {
    ...
    private void buildNavAmpBinding(Trigger trigger, boolean isRedAlliance) {
        ...
        Command swingBackAndReleaseCommand = new SetArmAngleCommand(mArm, ShootingParameters.AMP_LOWSPEED.angle_deg)
                .alongWith(new FeedCommand(mTransfer, mShooter));
        Command stopShootingCommand = new InstantCommand(() -> mShooter.stop())
                .andThen(new SetArmAngleCommand(mArm, ArmConstants.ARM_OBSERVE_ANGLE));
    ...
    private Command createArizonaSweepNearCommand() {
        return ArizonaAutoSweep.buildArizonaSweepCommand(
                sDrivetrainSubsystem, mArm, mShooter, mTransfer, mIntake, mArizonaSweepModeChooser.getSelected());
    }

    private Command createCaliforniaSweepMidFieldCommand() {
        return CaliforniaAutoScore515253.buildCaliforniaAutoScore515253(
