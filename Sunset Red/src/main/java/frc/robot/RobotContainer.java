...
public class RobotContainer {
  ...
  private final SendableChooser<ArizonaSweepMode> mArizonaSweepModeChooser =
      new SendableChooser<>();
  private final SendableChooser<CaliforniaStrategy> mCaliforniaStrategyChooser =
      new SendableChooser<>();
  // * Controllers */
  ...
  private void configureBindings() {
    ...
    buildShootBinding(operatorController.a(), ShootingParameters.BELOW_SPEAKER_REVERSE);
                    ...
                    new InstantCommand(() -> mTransfer.setVoltage(-1))
                        .andThen(
                            new WaitUntilCommand(() -> !mTransfer.isOmronDetected())
                                .raceWith(new WaitCommand(0.3)))) // note
                // got
                // in
    ...
    buildPepGBinding(
        new Trigger[] {
          ...
          rightStickLeft,
          rightStickRight,
          rightStickDown.and(driverController.rightBumper())
        });
  }
  ...
  public void pushChooser() {
    ...
    mCaliforniaStrategyChooser.setDefaultOption("Near Side 51, 52, 53", CaliforniaStrategy.DEFAULT);
    ...
    // Add Arizona Auto
    mChooser.addOption("Arizona Sweep Near", new ProxyCommand(this::createArizonaSweepNearCommand));

    // Add California Auto
    mChooser.addOption(
        "California Auto Score 51, 52, 53", new ProxyCommand(this::createCaliforniaAutoCommand));
  ...
  private void buildPepGBinding(Trigger[] triggers) {
    Function<GoalZone, PepGuardiolaCommand> buildGuardiola =
        (goalzone) ->
