...
public class ApriltagCoprocessor extends SubsystemBase {
  ...
  public record VisionObservation(
      EstimatedRobotPose estimate, double xyDev, double angleDev, String camname) {}
  ...
  private ArrayList<PhotonCamera> cameras =
      new ArrayList<>(
          Arrays.asList(
              ApriltagCamShooterSide, ApriltagCamIntakeSide, ApriltagCamShooterLongFocal));
  ...
  private static final double[] xyDevFactors =
      new double[] {0.01, 0.01, 0.02}; // 0.005, 0.005, 0.01
  ...
  private Optional<EstimatedRobotPose> processCameraResult(
      int camIdx, Pose2d prevEstimatedRobotPose) {
   ...
   */
  public List<VisionObservation> updateEstimatedGlobalPose(
    ...
    for (int i = 0; i < cameras.size(); i++) {
      ...
      if (!camresult.isPresent()) {
        SmartDashboard.putString(
            "Vision Update Status (" + cameras.get(i).getName() + ")", "NO_PROCESSED_RESULT");
        continue;
      }
      ...
      if (Math.abs(camresult.get().estimatedPose.getZ()) > Z_MARGIN) {
        SmartDashboard.putString(
            "Vision Update Status (" + cameras.get(i).getName() + ")", "Z_POSE_INCORRECT");
        continue;
      }
      ...
      if (lastVisionEstimatedPose[i] != null
        ...
        SmartDashboard.putString(
            "vision vel ms (" + cameras.get(i).getName(), visionVelMS.toString());
        ...
        if ((chassisVelMS.getNorm() < 0.5 && deltav > STATIC_DELTAV_BOUND)
          ...
          SmartDashboard.putString(
              "Vision Update Status (" + cameras.get(i).getName() + ")", "VELOCITY_INCORRECT");
          continue;
        }
      }
      ...
      VisionObservation newobs =
          new VisionObservation(camresult.get(), xyDev, angleDev, cameras.get(i).getName());
      allObservation.add(newobs);
      SmartDashboard.putString("Vision Update Status (" + cameras.get(i).getName() + ")", "OK");
      SmartDashboard.putString(
          "Vision Observation Log",
          newobs.toString() + " " + camresult.get().estimatedPose.toString());
  ...
  private List<PhotonTrackedTarget> filterAcceptableTargets(
      PhotonPipelineResult result, double minArea) {
