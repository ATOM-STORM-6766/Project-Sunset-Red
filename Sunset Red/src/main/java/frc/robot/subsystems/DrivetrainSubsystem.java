...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  private final CircularInterpolatingTreeMap<Double, Rotation2d> mHeading =
      new CircularInterpolatingTreeMap<>(100, // Adjust this size as needed
          InverseInterpolator.forDouble(), new Interpolator<Rotation2d>() {
            @Override
  ...
  StructArrayPublisher<SwerveModuleState> vision_pose_publisher =
      NetworkTableInstance.getDefault().getTable("SmartDashboard")
          .getStructArrayTopic("vision pose", SwerveModuleState.struct).publish();
  private final SwerveDriveModule[] mSwerveModules;
  private final SwerveDrivePoseEstimator mEstimator;
  private final Notifier mNotifier;

  StructPublisher<Pose2d> mPosePublisher = NetworkTableInstance.getDefault()
      .getTable("SmartDashboard").getStructTopic("RobotPose", Pose2d.struct).publish();
  ...
  private SwerveKinematicLimits mKinematicLimits =
      new SwerveKinematicLimits(DriveConstants.kPhysicalMaxSpeedMetersPerSecond * 0.95,
          DriveConstants.kPhysicalMaxSpeedMetersPerSecond / 0.3, 100);
  ...
  public DrivetrainSubsystem() {
    ...
    mSwerveModules = new SwerveDriveModule[] {new SwerveDriveModule(SwerveModuleConstants.FL),
    ...
    mKinematics = new SwerveDriveKinematics(mSwerveModules[0].getTranslationToRobotCenter(),
    ...
    mEstimator = new SwerveDrivePoseEstimator(mKinematics, getGyroYaw(), getModulePositions(),
        new Pose2d(), VecBuilder.fill(0.1, 0.1, 0.1), VecBuilder.fill(0.5, 0.5, 0.5)); // adjust for
                                                                                       // need
                                                                                       // (vision-related)
  ...
  public void initSendable(SendableBuilder builder) {
    ...
    builder.addDoubleProperty("ChassisSpeed.omega", () -> mKinematicSpeed.omegaRadiansPerSecond,
        null);
    builder.addDoubleProperty("FilteredSpeed.omega", () -> mFilteredSpeed.omegaRadiansPerSecond,
        null);
    ...
    builder.addDoubleArrayProperty("target chassis speeds x y omega",
        () -> new double[] {mTargetSpeeds.vxMetersPerSecond, mTargetSpeeds.vyMetersPerSecond,
  ...
  private void genSetpointAndApply(ChassisSpeeds des_cs) {
    ...
    Pose2d robot_pose_vel = new Pose2d(des_cs.vxMetersPerSecond * Constants.kPeriodicDt,
    ...
    ChassisSpeeds updated_chassis_speeds = new ChassisSpeeds(twist_vel.dx * kLooperFreq,
        twist_vel.dy * kLooperFreq, twist_vel.dtheta * kLooperFreq);
    mTargetSpeeds = updated_chassis_speeds;
    mSetpoint = mSetpointGenerator.generateSetpoint(mKinematicLimits, mSetpoint,
        updated_chassis_speeds, Constants.kPeriodicDt, this);
  ...
  private double compensate(StatusSignal<Double> pos_sig, StatusSignal<Double> pos_vel,
      double ctretime) {
    ...
    synchronized (mEstimator) {
      mEstimator.resetPosition(getGyroYaw(), getModulePositions(),
          new Pose2d(getPose().getTranslation(),
              DriverStation.getAlliance().orElse(Alliance.Blue) == Alliance.Blue ? new Rotation2d()
  ...
  public void periodic() {
    ...
    mKinematicSpeed = mKinematics.toChassisSpeeds(mSwerveModules[0].getState(),
        mSwerveModules[1].getState(), mSwerveModules[2].getState(), mSwerveModules[3].getState());
  ...
  private double updateOdomFromVision() {
    synchronized (mEstimator) {
      Optional<EstimatedRobotPose> visionEstimatedPose = ApriltagCoprocessor.getInstance()
          .updateEstimatedGlobalPose(mEstimator.getEstimatedPosition(), new Translation2d(
      ...
      if (visionEstimatedPose.isPresent()) {
        ...
        if (useIMUPose2d.getRotation() != null) {
          if (visionEstimatedPose.get().strategy == PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR) {
            mEstimator.addVisionMeasurement(estimatedPose2d, photonTimestamp,
                VecBuilder.fill(0.1, 0.1, 0.1));
          } else {
            mEstimator.addVisionMeasurement(useIMUPose2d, photonTimestamp,
  ...
  private Command runSingleModuleZeroing(SwerveDriveModule module) {
    ...
    return new SequentialCommandGroup(Commands.runOnce(module::startZeroing),
        new WaitUntilCommand(module::checkLightGate), Commands.runOnce(module::stopAndCalibrate))
  ...
  public Command runZeroingCommand() {
    Command ret =
        ...
        new ParallelCommandGroup(runSingleModuleZeroing(mSwerveModules[0]),
            runSingleModuleZeroing(mSwerveModules[1]), runSingleModuleZeroing(mSwerveModules[2]),
  ...
  public boolean allModuleZeroed() {
    return mSwerveModules[0].getIsZeroed() && mSwerveModules[1].getIsZeroed()
        && mSwerveModules[2].getIsZeroed() && mSwerveModules[3].getIsZeroed();
  }
  ...
  public void configureAutoBuilder() {
    ...
    AutoBuilder.configureHolonomic(this::getPose, // poseSupplier
        ...
        new HolonomicPathFollowerConfig(
            ...
            new PIDConstants(5.0, 0.0, 0.0), DriveConstants.kPhysicalMaxSpeedMetersPerSecond, // maxModuleSpeed
