...
public class LED extends SubsystemBase {
  ...
  private synchronized void setRainbow() {
    // int temp = Math.min(LED_LENGTH-1,(int)(LED_LENGTH *
    // Shooter.getInstance().getSpinningPercentage()));
    // if(temp >= 42){
  ...
  public synchronized void setRisingColor(Color color) {
    ...
    brightPosition += 1;
    if (brightPosition >= LED_LENGTH) brightPosition = 0;
    for (int i = 0; i < LED_LENGTH; i++) {
      mLEDBuffer.setRGB(i, 0, 0, 0);
    }
    for (int i = 0; i < TAIL_LENGTH; i++) {
      mLEDBuffer.setRGB(
          (brightPosition + LED_LENGTH + i) % LED_LENGTH,
          (int) (color8bit.red / TAIL_LENGTH * i),
          (int) (color8bit.green / TAIL_LENGTH * i),
          (int) (color8bit.blue / TAIL_LENGTH * i));
    }
  }
